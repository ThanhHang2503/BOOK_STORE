...
class CTPhieuNhapGUI:
    def __init__(self, parent):
        ...
        self.ds_chitiet = DSCTPhieuNhap()  # KẾT NỐI DATABASE
        self.current_maPN = None
        self.taoGiaoDien()
    ...
    def hienThiDS(self, maPN=None):
        try:
            ...
            if maPN:
                # Chỉ hiển thị chi tiết của hóa đơn có mã maPN
                self.current_maPN = maPN
                ket_qua = self.ds_chitiet.tim_theo_ma_pn(maPN)
                for ct in ket_qua:
                    self.tree.insert("", "end", values=(ct.maPN, ct.maSP, ct.soLuongSP, ct.donGia, ct.thanhTien))

                # Cập nhật tiêu đề frame danh sách
                self.frame_danh_sach.configure(text=f"Chi tiết của hóa đơn {maPN}")
    ...
    def hienThi(self, action):
        ...
        if action == "Hiển thị danh sách":
            ...
            if action == "Thêm chi tiết hóa đơn":
                ...
                if self.current_maPN:
                    self.entry_maPN.delete(0, tk.END)
                    self.entry_maPN.insert(0, self.current_maPN)
    ...
    def taoGiaoDien(self):
        ...
        self.entry_maPN.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        ...
        self.btn_luu = tk.Button(self.frame_button, text="OK", command=self.luuChiTietPN, bg="#4CAF50", fg="white")
        ...
        self.tree = ttk.Treeview(self.frame_danh_sach, columns=("maPN", "maSP", "soLuongSP", "donGia", "thanhTien"),
                                 show="headings", height=15)
        self.tree.heading("maPN", text="Mã hóa đơn")
        ...
        self.tree.column("maPN", width=100)
    ...
    def sua_chitiet_tu_bang(self):
        ...
        self.chonPN(None)  # Gọi hàm này để điền dữ liệu vào form
    ...
    def chonPN(self, event):
        ...
        if selected_items:
            ...
            if self.entry_maPN.cget("state") == "readonly":
                self.entry_maPN.config(state="readonly")
            ...
            if self.entry_maPN.cget("state") == "readonly" and self.entry_maSP.cget("state") == "readonly":
                self.btn_luu.config(state="normal")

    def luuChiTietPN(self):
        ...
        try:
            ...
            if not all([maPN, maSP, soLuongSP, donGia]):
            ...
            # Kiểm tra trạng thái của entry_maPN để xác định là thêm mới hay cập nhật
            if self.entry_maPN.cget("state") == "readonly" and self.entry_maSP.cget("state") == "readonly":
                # Đang ở chế độ sửa
                if self.ds_chitiet.xoa(maPN, maSP) and self.ds_chitiet.them(ct):
                    self.hienThiDS(self.current_maPN)  # Cập nhật hiển thị với cùng mã hóa đơn hiện tại
                    self.lam_moi_form()
                ...
                if self.ds_chitiet.them(ct):
                    ...
                    self.hienThiDS(maPN if maPN == self.current_maPN else self.current_maPN)
                    self.lam_moi_form()
    ...
    def xoaChiTietPN(self):
        ...
        try:
            ...
            confirm = messagebox.askyesno("Xác nhận xóa",
                                          f"Bạn có chắc muốn xóa chi tiết sản phẩm {maSP} của hóa đơn {maPN}?")
            if confirm:
                if self.ds_chitiet.xoa(maPN, maSP):
                    self.hienThiDS(self.current_maPN)  # Cập nhật hiển thị với cùng mã hóa đơn hiện tại
