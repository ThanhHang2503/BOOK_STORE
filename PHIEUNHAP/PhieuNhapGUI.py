...
class PhieuNhapGUI:
    ...
    def sua_phieu_nhap(self):
        ...
        ttk.Label(self.frame_thong_tin, text="Nhập Mã Phiếu Nhập:").grid(row=0, column=0, padx=5, pady=5)
        
        self.entry_ma_pn = ttk.Entry(self.frame_thong_tin, width=20)
        self.entry_ma_pn.grid(row=0, column=1, padx=5, pady=5)

        btn_tim = ttk.Button(self.frame_thong_tin, text="Sửa", command=self.tim_phieu_nhap)
        btn_tim.grid(row=0, column=2, padx=5, pady=5)

    def tim_phieu_nhap(self):
        maPN = self.entry_ma_pn.get().strip()
        if not maPN:
            messagebox.showwarning("Cảnh báo", "Vui lòng nhập mã phiếu nhập!")
            return

        du_lieu = self.ds_phieu_nhap.tim_kiem_phieu_nhap(maPN)  
        if not du_lieu:
            messagebox.showerror("Lỗi", "Không tìm thấy phiếu nhập!")
            return

        maNV, ngayTao = du_lieu[0]  # Giả sử chỉ có một dòng kết quả

        ttk.Label(self.frame_thong_tin, text="Sửa Mã Nhân Viên:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.entry_sua_maNV = ttk.Entry(self.frame_thong_tin)
        self.entry_sua_maNV.grid(row=1, column=1, padx=5, pady=5)
        self.entry_sua_maNV.insert(0, maNV)

        ttk.Label(self.frame_thong_tin, text="Sửa Ngày Tạo:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.entry_sua_ngayTao = ttk.Entry(self.frame_thong_tin)
        self.entry_sua_ngayTao.grid(row=2, column=1, padx=5, pady=5)
        self.entry_sua_ngayTao.insert(0, ngayTao)

        btn_sua_ctpn = ttk.Button(self.frame_thong_tin, text="Sửa Chi Tiết", command=lambda: self.sua_ctpn(maPN))
        btn_sua_ctpn.grid(row=3, column=0, padx=5, pady=5)

        btn_luu_sua = ttk.Button(self.frame_thong_tin, text="Lưu", command=lambda: self.luu_sua_phieu_nhap(maPN))
        btn_luu_sua.grid(row=3, column=1, padx=5, pady=5)

        self.frame_thong_tin.update_idletasks()

    def luu_sua_phieu_nhap(self, maPN):
        maNV_moi = self.entry_sua_maNV.get()
        ngayTao_moi = self.entry_sua_ngayTao.get()

        if not maNV_moi or not ngayTao_moi:
            messagebox.showerror("Lỗi", "Không được để trống thông tin!")
            return

        # Cập nhật dữ liệu vào SQL
        query = f"UPDATE PhieuNhap SET maNV = '{maNV_moi}', ngayTao = '{ngayTao_moi}' WHERE maPN = {maPN}"
        self.luu_du_lieu_vao_sql(query)

        messagebox.showinfo("Thành công", "Cập nhật phiếu nhập thành công!")

    def sua_ctpn(self, maPN):
        # Lấy dữ liệu chi tiết phiếu nhập
        du_lieu = self.ds_phieu_nhap.tim_kiem_phieu_nhap(maPN, chi_tiet=True)

        if not du_lieu:
            messagebox.showerror("Lỗi", "Không có chi tiết phiếu nhập!")
            return

        self.frame_ctpn = tk.Toplevel(self.frame_thong_tin)
        self.frame_ctpn.title("Sửa Chi Tiết Phiếu Nhập")

        ttk.Label(self.frame_ctpn, text="Mã Sản Phẩm").grid(row=0, column=0, padx=10, pady=5)
        ttk.Label(self.frame_ctpn, text="Số Lượng").grid(row=0, column=1, padx=10, pady=5)
        ttk.Label(self.frame_ctpn, text="Đơn Giá").grid(row=0, column=2, padx=10, pady=5)

        self.entries_ctpn = []
        for i, (maSP, soLuong, donGia) in enumerate(du_lieu, start=1):
            ttk.Label(self.frame_ctpn, text=maSP).grid(row=i, column=0, padx=10, pady=5)

            entry_soLuong = ttk.Entry(self.frame_ctpn)
            entry_soLuong.insert(0, soLuong)
            entry_soLuong.grid(row=i, column=1, padx=10, pady=5)

            entry_donGia = ttk.Entry(self.frame_ctpn)
            entry_donGia.insert(0, donGia)
            entry_donGia.grid(row=i, column=2, padx=10, pady=5)

            self.entries_ctpn.append((maSP, entry_soLuong, entry_donGia))

        btn_luu_ctpn = ttk.Button(self.frame_ctpn, text="Lưu", command=lambda: self.luu_sua_ctpn(maPN))
        btn_luu_ctpn.grid(row=len(du_lieu) + 1, column=1, padx=10, pady=10)

    def luu_sua_ctpn(self, maPN):
        for maSP, entry_soLuong, entry_donGia in self.entries_ctpn:
            soLuong_moi = entry_soLuong.get()
            donGia_moi = entry_donGia.get()

            if not soLuong_moi or not donGia_moi:
                messagebox.showerror("Lỗi", "Không được để trống thông tin!")
                return

            query = f"UPDATE ChiTietPhieuNhap SET soLuong = {soLuong_moi}, donGia = {donGia_moi} WHERE maPN = {maPN} AND maSP = {maSP}"
            self.luu_du_lieu_vao_sql(query)

        messagebox.showinfo("Thành công", "Cập nhật chi tiết phiếu nhập thành công!")
        self.frame_ctpn.destroy()
    ...
    def tim(self):
        ...
        self.frame_thong_tin.update_idletasks()  #cập nhật và xử lý ngay lập tức 
